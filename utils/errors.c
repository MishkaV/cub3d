...
/*   Updated: 2021/04/22 21:46:02 by jbenjy           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	put_error(int error)
{
	...
	if (error == ERROR_MAP_CLOSE)
		ft_putstr_fd("Error: Bad data in .cub file(not closed map)\n", 0);
	if (error == ERROR_MAP_SYM)
		ft_putstr_fd("Error: Bad data in .cub file(wrong sym in map)\n", 0);
...
void	free_arguments(t_global *global)
{
	...
	if (global->list_sprite)
		free_list_sprite(global->list_sprite);
	if (global->list_dist.list_dist)
		free(global->list_dist.list_dist);
}
